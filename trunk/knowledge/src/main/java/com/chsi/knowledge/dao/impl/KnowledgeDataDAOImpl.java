...
public class KnowledgeDataDAOImpl extends BaseHibernateDAO implements KnowledgeDataDAO {
    ...
    public void updateVisitCntPlus(String id, int count) {
        ...
        String hql = "update KnowledgeData p set p.visitCnt=p.visitCnt+"+count + W + ID;
        Query query = hibernateUtil.getSession().createQuery(hql).setString("id", id);
        query.executeUpdate();
    }
    ...
    @Override
    public List<KnowledgeData> get(String systemId, String tag, KnowledgeStatus dsh, String type, int start, int size) {
        // TODO Auto-generated method stub
    ...
    @Override
    public List<KnowledgeData> get(String systemId, String tag, KnowledgeStatus dsh, String type, String userId, int start, int size) {
        String hql = "select A from KnowledgeData A where 1=1 ";
        if (dsh != null) {
            hql += " and A.knowledgeStatus = " + dsh.getNum() + " ";
        }
