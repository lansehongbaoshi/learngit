...
<button type="button" id="searchBtn" class="btn btn-purple btn-sm">
    ...
    <script type="text/javascript">
        function showSearchResult(systemId, keywords, curPage) {
            $.getJSON("/query/searchAllKnow.action", {
                    systemId: systemId,
                    keywords: keywords,
                    tag:$("#tags").val(),
                    type:$("#type").val(),
                ...
                function showSearchResult(json) {
                    if (json.flag == 'true') {
                        ...
                        for (var i = 0; i < knows.length; i++) {
                            ...
                            var str = " <tr role=\"row\" data-id="+k.knowId+" class=\""+odd_even+"\"><td class=\"hidden-80\">" + k.system + "</td><td class=\"hidden-80\">" + k.tags + "</td><td class=\"hidden-160\">" + (k.hasImage?("<i class=\"ace-icon fa fa-image bigger-130\"></i>"+k.title):k.title) + "</td><td class=\"hidden-260\"><a target='_blank' href='/query/viewKnowledge.action?id=" + k.knowId + "'><div class='zxx_text_overflow_1' title='"+ k.contentTxt +"'>"  + k.contentTxt + "</div></a></td><td class=\"hidden-80\">"+k.sort+"</td><td class=\"hidden-80\">"+k.visitCnt+"</td><td class=\hidden-80\><span class=\"label label-sm label-success\">已发布</span><span class=\"label label-sm label-success\">"+k.type+"</span></td></tr>";
        ...
        $(function () {
            $.getJSON("/query/listSystem.action",
            ...
            $(document).on("change","#systemIds",function() {
            	...
            	$.getJSON("/query/listTag.action",data,
