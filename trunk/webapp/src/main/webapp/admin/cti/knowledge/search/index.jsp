...
<br>
       ...
       <div class="col-xs-12 col-md-2">
        ...
        <input id="keywords" type="text" class="form-control search-query" placeholder="知识标题、回答、标签、关键字..." name="keywords" />
...
<button type="button" id="searchBtn" class="btn btn-purple btn-sm">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <span class="col-xs-5" style="display: ">
                            <a  href="/admin/cti/knowledge/searchindex/addindex.action" target="_self"  class="btn-new-mail pull-right">
    ...
    <form id="modifyForm" action="/admin/cti/knowledge/searchindex/modifyindex.action" method="post" target="_self">
    <input type="hidden" id="theId" name="id">
    </form>
    <script type="text/javascript">
        function showSearchResult(systemId, keywords, curPage) {
            $.getJSON("/admin/cti/search/searchAllKnow.action", {
                    systemId: systemId,
                    keywords: keywords,
                    tag:$("#tags").val(),
                    type:$("#type").val(),
                    ...
                    if (json.flag == 'true') {
                        ...
                        for (var i = 0; i < knows.length; i++) {
                            ...
                            if(k.topTime==-1){//未置顶热点问题
                                var str = " <tr role=\"row\" data-id="+k.knowId+" class=\""+odd_even+"\"><td class=\"hidden-80\" title=\""+k.systems+"\">" + k.system + "</td><td class=\"hidden-80\">" + k.tags + "</td><td class=\"hidden-160\">" + (k.hasImage?("<i class=\"ace-icon fa fa-image bigger-130\"></i>&nbsp;"+k.title):k.title) + "</td><td class=\"hidden-260\"><div class='zxx_text_overflow_1' title='"+ k.contentTxt +"'>" + k.contentTxt + "<div></td><td class=\"hidden-80\">"+k.sort+"</td><td class=\"hidden-80\">"+k.visitCnt+"</td><td class=\hidden-80\><span class=\"label label-sm label-success\">已发布</span><span class=\"label label-sm label-success\">"+k.typeDic+"</span></td><td><div class=\"hidden-sm hidden-xs action-buttons\"><a class=\"blue\" target='_blank' title=\"查看\" href=\"/admin/cti/knowledge/searchindex/showKnowledge.action?id=" + k.knowId +"\"> <i class=\"ace-icon fa fa-search-plus bigger-130\"></i> </a>"+((k.type=="PRIVATE")?("<a class=\"green\" title=\"修改\" href='javascript:void(0)' class='modifyclass'><i class=\"ace-icon fa fa-pencil bigger-130\"></i> </a>"):"")+"</div></td></tr>";
                            }else{//已置顶热点问题
                            	var str = " <tr role=\"row\" data-id="+k.knowId+" class=\""+odd_even+"\"><td class=\"hidden-80\" title=\""+k.systems+"\">" + k.system + "</td><td class=\"hidden-80\">" + k.tags + "</td><td class=\"hidden-160\">" + (k.hasImage?("<i class=\"ace-icon fa fa-image bigger-130\"></i>&nbsp;"+k.title):k.title) + "</td><td class=\"hidden-260\">" + k.summary + "</td><td class=\"hidden-80\">"+k.sort+"</td><td class=\"hidden-80\">"+k.visitCnt+"</td><td class=\hidden-80\><span class=\"label label-sm label-success\">已发布</span><span class=\"label label-sm label-success\">"+k.typeDic+"</span></td><td><div class=\"hidden-sm hidden-xs action-buttons\"><a class=\"blue\" target='_blank' title=\"查看\" href=\"/admin/cti/knowledge/searchindex/showKnowledge.action?id=" + k.knowId +"\"> <i class=\"ace-icon fa fa-search-plus bigger-130\"></i> </a> "+((k.type=="PRIVATE")?("<a class=\"green\" title=\"修改\" href='javascript:void(0)' class='modifyclass'><i class=\"ace-icon fa fa-pencil bigger-130\"></i> </a>"):"")+"</div></td></tr>";
                            }
