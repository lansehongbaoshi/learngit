  ...
  <div class="row">
    <form name="" action="/htgl/robot/set/update.action" method="post" onsubmit="return checkForm(this)" enctype="multipart/form-data">
      ...
      <input type="hidden" name="id" value="<s:property value='key.id'/>">
        ...
        <p>
          匹配分词阀值：<input type="text" name="num" value="<s:property value="key.num"/>" check="^[1-9]\d*$" warn="关键词个数为正整数" style="width: 400px;"  maxlength="50" <s:if test="key.systemDefined">readonly="true"</s:if>><span>（说明：用户输入与系统配置提问匹配到的分词数目大于等于此值时才认为回答合理）</span>
        </p>
        <p>
